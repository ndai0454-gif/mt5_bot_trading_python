...
Strategy: M5 EMA Momentum + Sideway RSI + Positive DCA
"""
...
logging.basicConfig(
    level=logging.INFO,
    format="%(asctime)s [%(levelname)s] %(name)s: %(message) same",
    handlers=[
...
class ScalpingBot:
    def __init__(self, config: dict):
        ...
        self.trade_manager: TradeManager = None 
    ...
    def start(self):
        ...
        if not self.connector.is_connected():
            ...
            if not connected:
                self._log(f"Failed to connect to MT5.", "ERROR")
                return
        ...
        if not self._symbol_info:
            self._symbol_info = self.connector.get_symbol_info(self.symbol) or {
                ...
                "volume_min": 0.01, "volume_max": 100.0, "trade_contract_size": 100.0,
            }
        ...
        if acc:
            ...
            if not can_continue:
                self._log(f"CRITICAL: {reason}. Closing all!", "ERROR")
    ...
    def _tick(self):
        ...
        # ── IN_TRADE — Monitor and check for DCA Positive ──────────────────
        if self._state == STATE_IN_TRADE:
            if self.trade_manager.trade_count() == 0:
                self._state = STATE_COOLING_DOWN
                self._cooling_bars = self.cfg.get("cooling_down_candles", 5)
                self._update_dashboard_state()
            ...
            if tick:
                ...
                # Monitor SL/TP
                phigh = tick.get("ask") if tick["bid"] == price else tick.get("high", price) # Simplified High
                plow = tick.get("bid") if tick["ask"] == price else tick.get("low", price) # Simplified Low
                
                df = self.connector.get_ohlcv(self.symbol, self.cfg["timeframe"], self.cfg["ohlcv_bars"])
                events = self.trade_manager.monitor(price, phigh, plow, df, self.signal_engine)
                for ev in events:
            ...
            # --- TÍCH HỢP DCA DƯƠNG KHI ĐANG TRONG LỆNH ---
            # Thay vì 'return', chúng ta tiếp tục cho phép quét tín hiệu để nhồi lệnh
            ...
            df_h1 = self.connector.get_ohlcv(self.symbol, "H1", 200)
            
            if df_m5 is not None and df_h1 is not None:
                signal = self.signal_engine.get_signal(df_m5, spread, df_h1)
                direction = signal.get("direction", NEUTRAL)
                
                if direction != NEUTRAL:
                    # Kiểm tra điều kiện nhồi lệnh (Max 5 lệnh + Khoảng cách lãi)
                    if self.trade_manager.can_open_dca_trade(direction, price):
                        self._log(f"DCA Positive signal detected: {direction}. Adding layer...", "TRADE")
        ...
        if signal.get("direction", NEUTRAL) == NEUTRAL and force_entry_available:
            signal = self.signal_engine.get_forced_signal(df_m5, spread, self.cfg.get("paper_force_direction", "AUTO"))
        ...
        if self._state == STATE_SCANNING:
            ...
            self._update_dashboard_state()
            self._log(f"Entry ready: {direction} | Regime: {signal.get('market_regime', 'TREND')} | RSI={signal['rsi']:.1f}", "INFO")
    ...
    def _execute_entry(self, signal: dict, acc: dict):
        ...
        # Tính lot dựa trên Risk Manager
        lot = self.risk_manager.calculate_lot_size(
            balance, sl_distance, tick_value, volume_step, tick_size, volume_min, volume_max
        )

        # Nếu là lệnh DCA Dương, áp dụng lot multiplier từ config
        trades_same_dir = [t for t in self.trade_manager.get_trades() if t.direction == direction]
        if len(trades_same_dir) > 0:
            lot = round(lot * self.cfg.get("dca_lot_multiplier", 1.0), 2)

        self._log(f"Entering {direction} (Layer {len(trades_same_dir)+1}) | Lot={lot:.2f} | SL={sl:.2f}", "TRADE")

        ticket = self.connector.place_order(self.symbol, direction, lot, sl, tp3, self.magic)
        ...
        trade = ManagedTrade(
            ticket=ticket, symbol=self.symbol, direction=direction,
            entry_price=close_price, lot_total=lot, sl=sl,
            tp1=signal["tp1"], tp2=signal["tp2"], tp3=tp3,
    ...
    def _paper_force_entry_available(self) -> bool:
        if not self.paper_mode or not self.cfg.get("paper_force_entry", False): return False
        if self.cfg.get("paper_force_once_per_start", True) and self._paper_forced_trade_done: return False
        return True

    def _update_dashboard_state(self):
        if self._dashboard: self._dashboard.update_state(self._state)

    def _log(self, msg: str, level: str = "INFO"):
        logger.info(msg)
        if self._dashboard: self._dashboard.log(msg, level)
            ...
            while self.connector.is_connected():
                try:
                    ...
                    if tick and self._dashboard:
                        ...
                        if self.trade_manager:
                            pnls = {t.ticket: t.unrealized_pnl(mid) for t in self.trade_manager.get_trades()}
                            if pnls: self._dashboard.update_position_pnl(pnls)
...
def main():
    ...
    def on_start(): bot.start()
    def on_stop(): bot.stop()
    def on_close_all(): bot.emergency_close_all()
    ...
    def auto_connect():
        ...
        if connected:
            ...
            if acc:
                ...
                dashboard.log(f"Connected | Acc: {acc['login']} | Bal: {acc['balance']:.2f}{mode_tag}", "TRADE")
                dashboard.update_account(acc["balance"], acc["equity"], 0.0, 0)
                bot._start_price_ticker()
        else:
            dashboard.update_connection(False)
            dashboard.log("MT5 connection failed.", "ERROR")
